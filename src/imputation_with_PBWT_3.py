...
def iterate_over_positions(s, X, L):
    """
    ...
    t_current, t_prev = 0, 0                     # Number of alleles per site
    ...
    for k in range(N):
        ...
        t_current = max(2,len(set(xk_current)))
        print(f"# alleles at current position: {t_current}")
        if k!=0 and X[M-1][k] == -1: # NOTE: Run only if X[M-1][k] == -1 
            # Extract xk for the previous position from the reference panel (used to get matches)
            xk_prev = [X[hap][k-1] for hap in ak]
            t_prev = max(2,len(set(xk_prev)))
            print(f"xk_prev: {xk_prev}")
            print(f"# alleles at prev position: {t_prev}")
            # Step 1: Find long matches for the current position
            print("Finding matches...")
            matches_ = find_long_matches(xk_prev, t_prev, ak, dk, k, L)
            print(matches_)
        ...
        ak, dk = prefix_and_divergence(xk_current, t_current, ak, dk, k)
        print(ak, dk)
...
# - Set reference
vcf_file="/Users/kmlanderos/Documents/Johns_Hopkins/Fall_2024/Computational_Genomics_Sequences/Project/data/ref_1000_1000.vcf"
...
# - Set query
vcf_file="/Users/kmlanderos/Documents/Johns_Hopkins/Fall_2024/Computational_Genomics_Sequences/Project/data/target_1000_1000.vcf"
...
for index, query in enumerate(query_m1.values.tolist()):
    imputed_m1 += [iterate_over_positions(s=query, X=ref_m1, L=L)]
...
for index, query in enumerate(query_m2.values.tolist()): 
    print(f"# ----- Query #{index} ----- #") 
    imputed_m2 += [iterate_over_positions(s=query, X=ref_m1, L=L)]
